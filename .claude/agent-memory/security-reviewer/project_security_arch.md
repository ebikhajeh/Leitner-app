...
1. CRITICAL: `server/.env` contains real credentials (DB password, BETTER_AUTH_SECRET)
2. HIGH: `/api/me` returns full session object including session token — over-exposure
3. HIGH: No `BETTER_AUTH_SECRET` validation at startup
...
7. MEDIUM: Seed script placeholder password is weak ("password123")
